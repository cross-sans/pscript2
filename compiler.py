...
def compile_neo(file):
    with open(file, 'r') as f:
        code = f.read()

    # Replace "set" statements with simple variable assignments
    code = re.sub(r'set\s+(\w+)\s+to\s+(.+)', r'\1 = \2', code)

    # Replace "print" statements with Python's "print" function
    code = re.sub(r'print (.+)', r'print("\1")', code)
    code = re.sub(r'printf (.+)', r'print(f"{\1}")', code)

    # Replace "printf" statements with Python's f-strings
    #code = re.sub(r'printf\s+(.+)', r'print(f"\1")', code)
    ...
    # Wrap code in a "main" function and call it
    code = f'def main():\n{code}\n\nif __name__ == "__main__":\n    main()'
    #save the code into a file, with the .neo(or .nl if i change it) replaced by a .py
    with open(file.replace(".neo", ".py"), "w") as f:
        f.write(code)
